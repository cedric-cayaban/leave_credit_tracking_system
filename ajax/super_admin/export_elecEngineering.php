<?php
    ...
    $reportSql = $con->query("SELECT * FROM 
                                ...
                                WHERE employee.department = 5 AND employee_leave.status IN ('Accepted', 'Rejected')"
                            );

    
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename="electrical_engineer_report.csv"');
