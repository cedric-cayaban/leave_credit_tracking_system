<?php
    ...
    $reportSql = $con->query("SELECT * FROM 
                                ...
                                WHERE employee.department = 4 AND employee_leave.status IN ('Accepted', 'Rejected')"
                            );

    
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename="computer_engineering_report.csv"');
