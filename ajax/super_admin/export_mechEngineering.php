<?php
    ...
    $reportSql = $con->query("SELECT * FROM 
                                ...
                                WHERE employee.department = 8 AND employee_leave.status IN ('Accepted', 'Rejected')"
                            );

    
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename="mechanical_engineering_report.csv"');
